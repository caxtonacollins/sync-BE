...
export class LiquidityEventProcessorService {
  ...
  private async handleUserRegistered(evt: EventPayload) {
    ...
    this.logger.log(
      `User Registered: ${token_account_id} | Account: ${userUuid}`,
    );
  ...
  private async completeNearestPendingOrder(
    ...
    const exchangeRate = await this.exchangeRateService.getExchangeRateFor([
      tokenSymbolWithoutUSD.toLowerCase(),
      fiat_symbol.toLowerCase(),
    ]);
    ...
    const fiat_amount =
      (token_amount / Math.pow(10, 18)) * tokenRate * fiatRate;
    ...
    // Determine swap direction based on event name
    const eventName = evt.data.name;
    const isFiatToToken = eventName === EVENT_NAMES.FiatToTokenSwapExecuted;

    // Convert token_amount and fee to string/Decimal for safe operations
    const tokenAmountDecimal = new Decimal(String(token_amount));
    const feeStr = String(fee || '0');

    // Update swap order, crypto balance in transaction
    await this.prisma.$transaction(async (tx) => {
      // Update swap order status to completed
      await tx.swapOrder.update({
      ...
      // Update cryptoBalance based on swap direction
      if (isFiatToToken) {
        ...
        await tx.cryptoBalance.update({
          ...
          data: {
            available: { increment: tokenAmountDecimal },
          },
        });
      } else {
        ...
        await tx.cryptoBalance.update({
          ...
          data: {
            available: { decrement: tokenAmountDecimal },
          },
        });
      }
